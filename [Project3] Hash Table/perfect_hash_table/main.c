...
int main(int argc, const char* argv[])
{
    ...
    FILE* inputKeysFile;
    int* inputKeys;
    int* sizeTable; // to notate the size of each second table
    FILE* inputFile;
    FILE* outputFile;
    int i = 0;
    ...
    // make size table
    sizeTable = (int *) calloc(size, sizeof(int));
    for(i=0; i<size; i++) {
        inputKeys[i] = inputKeys[i] % size; // hash value
        sizeTable[inputKeys[i]]++;
    }
    ...
    HashTable* ht = makeHashTable(size*3, sizeTable);
